...
public class TypeMap_Base_Types_Only_10_Pct_Hits {

  public List<Class> classes = new ArrayList();
  public TypeMap basic;
  public TypeMap tree;
  public TypeMap graph;
  ...
  public void basic(Blackhole bh) {
    Object val = basic.get(requestedKeys.get(counter));
    bh.consume(val);
  }

  @Benchmark
  public void greedy(Blackhole bh) {
    Object val = greedy.get(requestedKeys.get(counter));
    bh.consume(val);
  }

  @Benchmark
  public void tree(Blackhole bh) {
    Object val = basic.get(requestedKeys.get(counter));
    bh.consume(val);
  }
  ...
  public void nativeTypeMap(Blackhole bh) {
    Object val = graph.get(requestedKeys.get(counter));
    bh.consume(val);
  }
  ...
  private List<Class> getKeysInMap(List<Class> classes) {
    ...
    return keys.subList(0, 1 + ((int) (keys.size() * .1)));
  }
  ...
  public void beforeIteration() {
    ...
    basic = TypeMap.fixedTypeMap(src);
    tree = TypeMap.treeTypeMap(src);
    greedy = TypeMap.greedyTypeMap(src);
    graph = TypeMap.nativeTypeMap(src);
